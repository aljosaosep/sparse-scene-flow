...
namespace SUN {
    namespace utils {
        ...
        Eigen::Vector3d Camera::BackprojectTo3D(const double u, const double v, const double depth) const {
            double u0 = this->K_(0,2);
            double v0 = this->K_(1,2);
            double f_u = this->K_(0,0);
            double f_v = this->K_(1,1);

            return Eigen::Vector3d((u - u0) / f_u * depth, (v - v0) / f_v * depth, depth);
        }
